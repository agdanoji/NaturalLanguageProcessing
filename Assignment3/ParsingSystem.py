...
class ParsingSystem:
    ...
    def apply(self, c, t):
        ...
        =================================================================
        """
        # right-arc implementation
        # if |s|>2 , add arc sj->si to A with label l and remove si from the stack.
        if t[0] == 'R':
            si=c.getStack(0)
            sj=c.getStack(1)
            c.removeTopStack()
            c.addArc(sj,si,t[2:-1])

        # left-arc implementation
        # if |s|>2 , add arc si->sj to A with label l and remove sj from the stack
        elif t[0]=='L':
            si = c.getStack(0)
            sj = c.getStack(1)
            c.removeSecondTopStack()
            c.addArc(si, sj, t[2:-1])
        else:
            c.shift()
